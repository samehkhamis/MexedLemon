...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	// Do stuff
	CapacityScaling<SmartDigraph, double> capacityscaling(g);
	if (c) capacityscaling.costMap<ArcMap>(cost);
	if (l) capacityscaling.lowerMap<ArcMap>(lower);
	if (u) capacityscaling.upperMap<ArcMap>(upper);
	if (s) capacityscaling.supplyMap<NodeMap>(supply);
	capacityscaling.run();
	...
	if (nlhs > 0)
	{
		...
		total[0] = capacityscaling.totalCost();
	}
	if (nlhs > 1)
	{
		ArcMap flowMap(g);
		capacityscaling.flowMap(flowMap);
	...
	{
		NodeMap potMap(g);
		capacityscaling.potentialMap(potMap);
