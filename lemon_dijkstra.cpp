...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	// Do stuff
	Dijkstra<SmartDigraph, ArcMap> dijkstra(g, len);
	if (t != 0)
		dijkstra.run(g.nodeFromId(s - 1), g.nodeFromId(t - 1));
	else
		dijkstra.run(g.nodeFromId(s - 1));
	
	// Create output
	if (nlhs > 0)
	{
		const NodeMap &distMap = dijkstra.distMap();
	...
	if (nlhs > 1)
	{
		const SmartDigraph::NodeMap<SmartDigraph::Arc> &predMap = dijkstra.predMap();
