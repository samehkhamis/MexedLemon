...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	// Do stuff
	MaxWeightedMatching<SmartGraph, EdgeMap> mwm(g, weight);
	mwm.run();
	...
	if (nlhs > 0)
	{
		...
		value[0] = mwm.matchingWeight();
	}
	if (nlhs > 1)
	{
		const SmartGraph::NodeMap<SmartGraph::Arc> &matchingMap = mwm.matchingMap();
