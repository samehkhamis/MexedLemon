...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	// Do stuff
	CostScaling<SmartDigraph, double> costscaling(g);
	if (c) costscaling.costMap<ArcMap>(cost);
	if (l) costscaling.lowerMap<ArcMap>(lower);
	if (u) costscaling.upperMap<ArcMap>(upper);
	if (s) costscaling.supplyMap<NodeMap>(supply);
	costscaling.run();
	...
	if (nlhs > 0)
	{
		...
		total[0] = costscaling.totalCost();
	}
	if (nlhs > 1)
	{
		ArcMap flowMap(g);
		costscaling.flowMap(flowMap);
	...
	{
		NodeMap potMap(g);
		costscaling.potentialMap(potMap);
